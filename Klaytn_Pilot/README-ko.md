...
본 프로젝트는 닛픽에서 운영하는 ‘불편함’ 앱 (https://www.nitpick.co.kr/)에 데이터를 기록하고 보상을 제공하는 스킴을 Klaytn 플랫폼 위에 구현하는 프로젝트로, 유저는 일상 속에서 경험하는 불편들을 앱에 올리고 이에 대한 보상 (Social Innovator Token과 포인트)을 받는다. Social Innovator Token은 앱 내에서 레벨을 올리고 닉네임을 변환할 수 있는 권한을 구매하는 수단이며, 포인트는 상점에서 다양한 아이템과 기프티콘을 구매할 수 있는 수단이다. 레벨이 올라가면 포인트 적립율이 향상되고 상점에서 포인트로 구매할 수 있는 상품의 옵션이 확대된다. 향후에는 블록체인에 기록되는 유저들의 불편한 경험 내용이 어디에, 어떻게 활용되었는지 추적하여 유저에게 다회의 보상을 제공하는 방식으로 발전이 가능하다.
...
또한, 효율적인 Gas사용을 위하여 User가 마지막으로 기록한 시간 `UserData.lastTimeStamp`를 Day로 변환하고, 현재 TimeStamp를 Day로 변환하여 비교하였다. Day 같다면 Month를 비교하였다. year의 경우 발생할 확률에 비하여 조건검사로 소모되는 Gas비용이 크다고 판단하여 비교하지 않았다.   
